...
function query(collectionName) {
    ...
    if (collectionName === 'books' && !collection[0]._id) {
        collection.forEach(book => {
            console.log('collection books: ', book);
            book._id = book.id
            delete book.id
            book.saleInfo.price = {}
            book.saleInfo.price.amount = Utils.getRandomInt(1, 100)
            book.saleInfo.price.currencyCode = ['EUR', 'USD', 'ILS'][Utils.getRandomInt(0, 2)]
        })
    }
