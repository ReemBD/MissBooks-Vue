...
const KEY = 'books';
const API_KEY = 'AIzaSyCvwYFVpbXMd3n25Z7Jfe-7zC-_0OKUgtM'
export default {
  query,
  getById: get,
  remove,
...
async function query() {
  ...
  if (!books || !books.length) {
    ...
    const res = await axios.get('https://www.googleapis.com/books/v1/volumes?printType=books&q=effective%20javascript')
...
async function getGoogleBooks(params = {}) {
  const queryStr = new URLSearchParams({
    ...
    .toString()
  const res = await axios.get(`https://www.googleapis.com/books/v1/volumes?${queryStr}`)
  return res.data
}

window.googleBooks = getGoogleBooks
...
function removeReview(reviewId, book) {
  const reviewIdx = book.reviews.findIndex(id => id === reviewId)
  const bookCopy = { ...book }
  bookCopy.reviews.splice(reviewIdx, 1)
  save(bookCopy)
  return bookCopy
}
